...
export function openMei(file = defaultMeiFileName) {
  if (typeof file === "string") {
    ...
    readingPromise.then(
      function(mei) {
        if (mei.includes('<mei')) { // a hint that it is a MEI file
          console.log('MEI file loading: ' + meiFileName);
          cm.setValue(mei);
          if (vrvWorker)
            vrvWorker.postMessage({
              'cmd': 'setOptions',
        ...
        } else if (mei.includes('<score-partwise')) { // compatible MusicXML
          console.log('MusicXML file loading: ' + meiFileName);
          vrvWorker.postMessage({
            'cmd': 'loadXmlData',
            'msg': mei
          });
      ...
      function() {
        console.error('Loading dragged file ' + meiFileName + ' failed.')
      }
    );
  }
}
...
// let fc = document.querySelector('.friendContainer');
