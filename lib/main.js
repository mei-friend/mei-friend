...
export function openMei(meiFileName = defaultMeiFileName) {
  console.info('openMei ' + meiFileName + ', ', cm);
  if (typeof meiFileName === "string") {
    fetch(meiFileName)
    ...
    let readingPromise = new Promise(function(loaded, notLoaded) {
      ...
      reader.onload = (event) => {
        ...
        console.info('Reader read ' + meiFileName.name + ', mei: ', mei);
        if (mei) loaded(mei);
        else notLoaded();
      }
      reader.readAsText(meiFileName);
    })
    readingPromise.then(
      function(mei) {
        cm.setValue(mei);
      ...
      function() {
        console.error('Loading dragged file ' + meiFileName.name + ' failed.')
      }
    );
  }
}
