...
export default class Viewer {
  ...
  // set Verovio options
  setVerovioOptions(newOptions = {}, loadData = true, redoLayout = false) {
    ...
    console.info('setVerovioOptions: ', this.vrvOptions);
    ...
    this.worker.postMessage({
      ...
      'redoLayout': redoLayout // boolean? skip loading data and do redoLayout
    });
  }
  ...
  redoLayout() {
    console.info('redoLayout(): ' + this.currentPage + ' of ' + this.pageCount);
    this.worker.postMessage({
      'cmd': 'redoLayout',
      'msg': 'unused'
    });
  ...
  navigate(cm, incElName = 'note', dir = 'forwards') {
    ...
    if (!measure) {
      ...
      if (incElName == 'note' || incElName == 'measure') {
        ...
        if (!id && dir == 'forwards') {
          this.doCursorUpdate = true;
          // this.updatePage(cm, 'forwards');
          if (!this.changeCurrentPage(dir)) return;
          let svg = this.navigateBeyondPage(cm, this, this.currentPage,
            dir, elsList, stNo, lyNo, y);
          // this.displaySVG(cm, svg);
        ...
        // this.findClosestNoteInChord(id, y);
      }
  ...
  navigateBeyondPage(cm, v, page, dir, els, stNo, lyNo, y) {
    ...
    promise.then(
      function(svg) {
        ...
        v.displaySVG(cm, svg);
        // if (dir == "forwards")
        //   id = dutils.getInFirstMeasure(document.querySelector('.measure'),
        //     els, stNo, lyNo);
        // if (dir == "backwards") {
        //   id = dutils.getInLastMeasure(document.querySelector('.measure'),
        //     els, stNo, lyNo);
        //   // check measure...
        //   v.findClosestNoteInChord(id, y)
        // }
      }
    );
