...
export function createControlsMenu(parentElement, scale) {
  ...
  let codeflipBtn = document.createElement('button');
  codeflipBtn.id = "code-update-btn";
  codeflipBtn.classList.add('btn');
  codeflipBtn.classList.add('icon');
  codeflipBtn.classList.add('icon-file-symlink-file'); // icon-alignment-aligned-to
  codeflipBtn.classList.add('inline-block-tight');
  addToolTip(codeflipBtn, {
    title: 'Re-render encoding manually'
  });
  // codeflipBtn.innerHTML = 'Redo';
  codeflipBtn.setAttribute('type', 'button');
  codeflipBtn.setAttribute('value', 'codeUpdate');
  codeflipBtn.disabled = true;
  updateCtrls.appendChild(codeflipBtn);
