...
* New "Let's Encode!" mode, entered with the `le_taskid` URL parameter alongside `file`, whose repository names the campaign: logs the volunteer in to the handed-over repository, branch, and file through the GitHub integration so their work can be committed, turns the git menu into a "Task Management" menu, and returns them to the campaign at `lets-encode.mdw.ac.at` with the outcome (`mf_status`, and `mf_msg` on failure). Failed commits are retried automatically, then leave the choice between another attempt and returning without committing.
