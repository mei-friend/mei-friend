...
### 1.6.0 Let's Encode! integration (develop)
* Splash text updated to acknowledge Let's Encode! funding (for all users) and to provide LE-specific context when arriving from Let's Encode (see below). New letsEncodeSplashDate allows updating LE-only splash content in future
* New "Let's Encode!" mode, entered with the `le_campaignname` and `le_taskid` URL parameters alongside `file`: logs the volunteer in to the handed-over repository, branch, and file through the GitHub integration so their work can be committed, turns the git menu into a "Task Management" menu, and returns them to the campaign they came from (`le_base` if given, else the referrer, else `lets-encode.mdw.ac.at`) with the outcome (`mf_status`, and `mf_msg` on failure). Failed commits are retried automatically, then leave the choice between another attempt and returning without committing.
* In Let's Encode mode, a session is scoped to its browser tab (`sessionStorage`) instead of sharing local storage with ordinary use, so working on a task doesn't overwrite the encoding, file location, or GitHub binding in regular mei-friend sessions. First step towards [#79](https://github.com/mei-friend/mei-friend/issues/79). The task parameters are restored to the address bar after the GitHub login strips them, so reloading stays in task; loading mei-friend without the params restores regular mode incl. previous session (localstorage)
* "Save" feature commits the work without completing task (handing over to LE platform); autosave commits every N minutes if changes exist, configurable in new "Let's Encode!" settings section (mei-friend tab, only shows in LE mode)
* When the task repo's lock table records an `expires` for the task, the time remaining to complete it is shown after the account name, with the expiry moment on hover. 
* In LE mode, GitHub menu replaced by a "Task Management" menu offering "Save," "Complete task," "Abandon task." The status line carries the campaign, task and GitHub account in place of the file name, location and changed marker, none of which mean anything when there is one file that is committed by completing the task. Completing and saving each write pre-fab, version-stamped commit messages. Completing and abandoning both confirm first, so misclicks don't lose work; abandoning also signs out of GitHub IFF mei-friend was logged in through this task
* Fix raw GitHub URL parsing to require a separator after the branch name, so a branch whose name prefixes another (e.g. `encode-task-1` and `encode-task-10`) is no longer mistaken for it
