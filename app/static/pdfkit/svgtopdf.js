...
var SVGtoPDF = function (doc, svg, x, y, options) {
        ...
        if (parser.match(/^>/)) {
          ...
          if ((temp = parser.match(/^<\/([\w:.-]+)\s*>/, true))) {
            ...
            if (temp[1] === node.nodeName) {
              ...
              warningCallback(
                'parseXml: tag not matching, opening "' + node.nodeName + '" & closing "' + temp[1] + '"'
              );
              error = true;
              return node;
            }
  ...
  function matchesSelector(elem, selector) {
    ...
    for (let i = 0; i < selector.classes.length; i++) {
      if (!elem.classList.includes(selector.classes[i])) {
        return false;
      }
    }
    return true;
  ...
  var BezierSegment = function (p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y) {
    ...
    this.startPoint = [
      p1x,
      p1y,
      isEqual(p1x, c1x) && isEqual(p1y, c1y) ? Math.atan2(c2y - c1y, c2x - c1x) : Math.atan2(c1y - p1y, c1x - p1x),
    ];
    this.endPoint = [
      p2x,
      p2y,
      isEqual(c2x, p2x) && isEqual(c2y, p2y) ? Math.atan2(c2y - c1y, c2x - c1x) : Math.atan2(p2y - c2y, p2x - c2x),
    ];
  ...
  var SvgShape = function () {
    ...
    this.A = function (rx, ry, fi, fa, fs, x, y) {
      if (isEqual(rx, 0) || isEqual(ry, 0)) {
        ...
        let r = Math.sqrt(
            Math.max(0, rx * rx * ry * ry - rx * rx * y1 * y1 - ry * ry * x1 * x1) /
  ...
  var SvgElem = function (obj, inherits) {
    ...
    this.getParentViewport = function () {
      return Math.sqrt(
        0.5 * this.getParentVWidth() * this.getParentVWidth() + 0.5 * this.getParentVHeight() * this.getParentVHeight()
      );
    };
  ...
  var SvgElemSymbol = function (obj, inherits) {
    ...
    this.getTransformation = function () {
      return multiplyMatrix(parseAspectRatio(aspectRatio, width, height, viewBox[2], viewBox[3]), [
        1,
        0,
        0,
        1,
  ...
  var SvgElemPattern = function (obj, inherits, fallback) {
    ...
    this.getPaint = function (bBox, gOpacity, isClip, isMask) {
      ...
      let viewBox = this.getViewbox('viewBox', [0, 0, width, height]),
        ...
        aspectRatioMatrix = multiplyMatrix(parseAspectRatio(aspectRatio, width, height, viewBox[2], viewBox[3], 0), [
          1,
          0,
          0,
          1,
  ...
  var SvgElemMarker = function (obj, inherits) {
    ...
    this.drawMarker = function (isClip, isMask, posArray, strokeWidth) {
      ...
      if (this.get('overflow') === 'hidden') {
        doc
          .rect(
            aspectRatioMatrix[0] * (viewBox[0] + viewBox[2] / 2 - refX) - width / 2,
            aspectRatioMatrix[3] * (viewBox[1] + viewBox[3] / 2 - refY) - height / 2,
            width,
            height
          )
          .clip();
  ...
  var SvgElemTextContainer = function (obj, inherits) {
    ...
    this.drawTextInDocument = function (isClip, isMask) {
      ...
      if (this.get('text-decoration') === 'line-through') {
        this.decorate(
          0.05 * this._font.size,
          0.5 * (getAscent(this._font.font, this._font.size) + getDescent(this._font.font, this._font.size)),
          isClip,
          isMask
        );
      }
    ...
    (function (textParentElem) {
      ...
      function doAnchoring() {
        if (currentChunk.length) {
          ...
          let anchordx =
            { startltr: 0, middleltr: 0.5, endltr: 1, startrtl: 1, middlertl: 0.5, endrtl: 0 }[
              currentAnchor + currentDirection
            ] * width || 0;
      ...
      function recursive(currentElem, parentElem) {
        ...
        let textLength = currentElem.getLength('textLength', currentElem.getVWidth(), undefined),
          ...
          baselineAttr = isTextElem
            ...
            : currentElem.get('alignment-baseline') || currentElem.get('dominant-baseline'),
