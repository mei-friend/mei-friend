...
export const lang = {
  ...
  splashBody: {
    html: `
      ...
      <p>
        Sebbene mei-friend sia un'applicazione basata su browser, i tuoi dati personali (inclusi la codifica che stai modificando e le impostazioni dell'applicazione) sono memorizzati nel <a href="https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage" target="_blank">local storage</a> del tuo browser e non sono memorizzati sui nostri server. Per proteggere le tue credenziali di accesso a GitHub, la tua sessione di accesso a GitHub (incluso il tuo token di accesso) è conservata sul server di mei-friend anziché nel tuo browser; il tuo browser conserva soltanto un identificatore di sessione opaco.
      </p>
      <p>
        I dati vengono trasmessi a GitHub solo quando lo richiedi esplicitamente (ad esempio, quando accedi a GitHub, carichi la tua codifica da o fai commit su un repository GitHub, o quando richiedi l'esecuzione di un workflow di GitHub Action per te). Allo stesso modo, i dati vengono trasmessi al provider Solid scelto solo quando lo richiedi esplicitamente (ad esempio, quando accedi a Solid, o carichi o salvi annotazioni stand-off). Per mantenere al sicuro le tue credenziali di accesso, tutte le interazioni con GitHub vengono instradate attraverso un server proxy ospitato dalla mdw – Università di Musica e Arti dello Spettacolo di Vienna, che allega le tue credenziali a queste richieste a partire dalla tua sessione di accesso. Questo server funge da intermediario tra il tuo browser e GitHub, e non memorizza il contenuto trasmesso attraverso di esso; come per qualsiasi server web, i metadati delle richieste (come i nomi dei repository e dei file a cui accedi) possono comparire nei suoi log tecnici.
      </p>
      <p>
  ...
  letsEncodeSaveButton: { text: 'Salva' },
